<?php defined('SYSPATH') OR die('No direct access allowed.');
/**
 * ORM Auth driver.
 *
 * $Id: ORM.php 4331 2009-05-06 23:30:40Z kiall $
 *
 * @package    Auth
 * @author     Kohana Team
 * @copyright  (c) 2007-2008 Kohana Team
 * @license    http://kohanaphp.com/license.html
 */
class Auth_ORM_Driver extends Auth_Driver {

	/**
	 * Checks if a session is active.
	 *
	 * @param   string   role name
	 * @param   array    collection of role names
	 * @return  boolean
	 */
	public function logged_in($role)
	{
		$status = FALSE;

		// Get the owner from the session
		$owner = $this->session->get($this->config['session_key']);

		if (is_object($owner) AND $owner instanceof Owner_Model AND $owner->loaded)
		{
			// Everything is okay so far
			$status = TRUE;

			if ( ! empty($role))
			{

				// If role is an array
				if (is_array($role))
				{
					// Check each role
					foreach ($role as $role_iteration)
					{
						if ( ! is_object($role_iteration))
						{
							$role_iteration = ORM::factory('role', $role_iteration);
						}
						// If the owner doesn't have the role
						if( ! $owner->has($role_iteration))
						{
							// Set the status false and get outta here
							$status = FALSE;
							break;
						}
					}
				}
				else
				{
				// Else just check the one supplied roles
					if ( ! is_object($role))
					{
						// Load the role
						$role = ORM::factory('role', $role);
					}

					// Check that the owner has the given role
					$status = $owner->has($role);
				}
			}
		}

		return $status;
	}

	/**
	 * Logs a owner in.
	 *
	 * @param   object   owner
	 * @param   string   password
	 * @param   boolean  enable auto-login
	 * @return  boolean
	 */
	public function login($owner, $password, $site_id, $remember)
	{
		// If the passwords match, and owner hass access to this site
		// perform a login
		#if ($owner->has(ORM::factory('role', 'login')) AND $owner->password === $password)
		if ($owner->has(ORM::factory('site', $site_id)) AND $owner->password === $password)
		{
			if ($remember === TRUE)
			{
				// Create a new autologin token
				$token = ORM::factory('owner_token');

				// Set token data
				$token->owner_id = $owner->id;
				$token->expires = time() + $this->config['lifetime'];
				$token->save();

				// Set the autologin cookie
				cookie::set('authautologin', $token->token, $this->config['lifetime']);
			}

			// Finish the login
			$this->complete_login($owner);

			return TRUE;
		}

		// Login failed
		return FALSE;
	}

	/**
	 * Forces a owner to be logged in, without specifying a password.
	 *
	 * @param   mixed    ownername
	 * @return  boolean
	 */
	public function force_login($owner)
	{
		if ( ! is_object($owner))
		{
			// Load the owner
			$owner = ORM::factory('owner', $owner);
		}

		// Mark the session as forced, to prevent owners from changing account information
		$_SESSION['auth_forced'] = TRUE;

		// Run the standard completion
		$this->complete_login($owner);
	}

	/**
	 * Logs a owner in, based on the authautologin cookie.
	 *
	 * @return  boolean
	 */
	public function auto_login()
	{
		if ($token = cookie::get('authautologin'))
		{
			// Load the token and owner
			$token = ORM::factory('owner_token', $token);

			if ($token->loaded AND $token->owner->loaded)
			{
				if ($token->user_agent === sha1(Kohana::$owner_agent))
				{
					// Save the token to create a new unique token
					$token->save();

					// Set the new token
					cookie::set('authautologin', $token->token, $token->expires - time());

					// Complete the login with the found data
					$this->complete_login($token->owner);

					// Automatic login was successful
					return TRUE;
				}

				// Token is invalid
				$token->delete();
			}
		}

		return FALSE;
	}

	/**
	 * Log a owner out and remove any auto-login cookies.
	 *
	 * @param   boolean  completely destroy the session
	 * @return  boolean
	 */
	public function logout($destroy)
	{
		if ($token = cookie::get('authautologin'))
		{
			// Delete the autologin cookie to prevent re-login
			cookie::delete('authautologin');
			
			// Clear the autologin token from the database
			$token = ORM::factory('owner_token', $token);
			
			if ($token->loaded)
			{
				$token->delete();
			}
		}

		return parent::logout($destroy);
	}

	/**
	 * Get the stored password for a ownername.
	 *
	 * @param   mixed   ownername
	 * @return  string
	 */
	public function password($owner)
	{
		if ( ! is_object($owner))
		{
			// Load the owner
			$owner = ORM::factory('owner', $owner);
		}

		return $owner->password;
	}

	/**
	 * Complete the login for a owner by incrementing the logins and setting
	 * session data: owner_id, ownername, roles
	 *
	 * @param   object   owner model object
	 * @return  void
	 */
	protected function complete_login($owner)
	{
		// Update the number of logins
		$owner->logins += 1;

		// Set the last login date
		$owner->last_login = time();

		// Save the owner
		$owner->save();

		return parent::complete_login($owner);
	}

} // End Auth_ORM_Driver